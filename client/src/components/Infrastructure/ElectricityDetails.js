...
export default class ElectricityDetails extends Component{
    constructor(){
        super();
        this.state={
             month:"",
    ...
    async change(e) {
      ...
      axios.get(`http://localhost:3001/infrastructure/getElectricityDetails?location=San Jose&month=${this.state.month}`)
       .then(res => {
    ...
    handleSave(e){
        ...
        this.state.records.map((record,index) => {
            axios.post("http://localhost:3001/infrastructure/saveElectricityDetails",{
                location: record.location,
                month: record.month,
                division_name: record.division_name,
                power_consumed: record.power_consumed,
    ...
    async handleDelete(e){
        ...
        axios.get(`http://localhost:3001/infrastructure/deleteElectricityDetails?location=San Jose&month=${this.state.month}&division_name=${removeContent.division_name}`)
       .then(res => {
            console.log(res.data);
            this.setState({
                msg: res.data
        ...
        return(
          <Form>
              <div className="alt-rentd-header">
                 <p className="alt-rentd-heading">Electricity Costs</p>
              ...
              <div className="alt-rentd-data">
                <label className="alt-rentd-data-label">Name of the division</label>
                <label className="alt-rentd-data-label">Power Consumed(in kW)</label>
                <label className="alt-rentd-data-label">amount (in $)</label>
              </div>
              <div className="alt-rentd-fields">
              {this.state.records.map((record,index) => (
                  <div className="alt-rentd-records">
                      <input className="alt-rentd-records-input1" onChange={this.handleChange} name="division_name" value={record.division_name} id={index} />
                      <input className="alt-rentd-records-input2" onChange={this.handleChange} name="power_consumed"  value={record.power_consumed} id={index} /> 
              ...
              <div className="alt-rentd-txt">
                  <p>{this.state.msg}</p>
                  <button onClick={this.handleAdd} className="alt-electricityd-add-btn">Add a New Record</button>
              </div>
          </Form>
        )
    }
