...
export default class RentDetails extends Component{
    ...
    handleSave(e){
        ...
        this.state.records.map((record,index) => {
            axios.post("http://localhost:3001/infrastructure/saveRentDetails",{
    ...
    async handleDelete(e){
        ...
        axios.get(`http://localhost:3001/infrastructure/deleteRentDetails?location=San Jose&month=${this.state.month}&building_name=${removeContent.building_name}`)
       .then(res => {
            console.log(res.data);
            this.setState({
                msg: res.data
        ...
        return(
          <Form>
              ...
              <div className="alt-rentd-data">
                <label className="alt-rentd-data-label">Name of the Building </label>
                <label className="alt-rentd-data-label">Area(in Sqft)</label>
                <label className="alt-rentd-data-label">amount per month(in $)</label>
              </div>
              <div className="alt-rentd-fields">
              {this.state.records.map((record,index) => (
                  <div className="alt-rentd-records">
                      <input className="alt-rentd-records-input1" onChange={this.handleChange} name="building_name" value={record.building_name} id={index} />
